				...
				for cpu in [cores]:
					for cpu_max_fq in cpu_freqs:
						for gpu_max_fq in gpu_freqs:
							...
							process1 = subprocess.Popen(command_logstart.format(low_pow_file + ".txt",low_pow_file, model, fl, iter, cpu, cpu_max_fq, gpu_max_fq),stdin=subprocess.PIPE,shell=True)
